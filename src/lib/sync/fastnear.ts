        ...
        .update({
          raw_data: {
            ...
            fastnear: {
              ...
              ft_contracts: (data.tokens || []).slice(0, 20).map((t) => t.contract_id),
              nft_contracts: (data.nfts || []).slice(0, 20).map((n) => n.contract_id),
              pool_ids: (data.pools || []).slice(0, 20).map((p) => p.pool_id),
