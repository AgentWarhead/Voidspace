...
export default function LegalRegulatoryBasics({ isActive, onToggle }: LegalRegulatoryBasicsProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (progress['legal-regulatory-basics']) setCompleted(true);
    } catch {}
  }, []);
  ...
  const handleComplete = () => {
    ...
    try {
      const progress = JSON.parse(localStorage.getItem('voidspace-founder-progress') || '{}');
      progress['legal-regulatory-basics'] = true;
    ...
    <Card variant="glass" padding="none" className="border-near-green/20">
      <div onClick={() => {}} style={{display:"none"}} className="cursor-pointer p-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
        ...
        <div className="flex items-center gap-3">
          <Badge className="bg-near-green/10 text-near-green border-near-green/20">Module 8 of 12</Badge>
