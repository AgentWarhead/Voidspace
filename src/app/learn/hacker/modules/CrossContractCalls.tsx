...
const CrossContractCalls: React.FC<CrossContractCallsProps> = ({ isActive, onToggle }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (progress['cross-contract-calls']) setCompleted(true);
    } catch {}
  }, []);
  ...
  const handleComplete = () => {
    ...
    try {
      const progress = JSON.parse(localStorage.getItem('voidspace-hacker-progress') || '{}');
      progress['cross-contract-calls'] = true;
    ...
    <Card variant="glass" padding="none" className="border-indigo-500/20">
      ...
      <div onClick={onToggle} className="cursor-pointer p-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
        ...
        <div className="flex items-center gap-3">
          <Badge className="bg-near-green/10 text-near-green border-near-green/20">Module 2 of 16</Badge>
